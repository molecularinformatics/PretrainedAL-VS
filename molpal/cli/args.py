...
def add_general_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument("--work-dir", help="the absolute path to the work directory")
    
    parser.add_argument("--clear-tensorboard-dir", help="whether to clear the tensorboard directory", action="store_true", default=False)
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="the number of GPUs for surrogate model training and inference. Default=1",
    )
...
def add_model_args(parser: ArgumentParser):
    ...
    model_parser.add_argument(
        "--model", choices=("rf", "gp", "nn", "mpn", "lgbm", "transformer", "molclr"), default="rf", help="the model type to use"
    )
    model_parser.add_argument(
        "--epochs", type=int, default=30, help="epoch to stop training the model"
    )
    model_parser.add_argument(
        "--test-batch-size",
        type=int,
...
def clean_and_fix_args(args: Namespace):
    ...
    if args.model != "lgbm":
        args_to_remove |= {"n_estimators", "max_depth"}
    if args.model != "gp":
        args_to_remove |= {"gp_kernel"}
    if args.model != "nn":
        args_to_remove |= set()
    if args.model != "mpn" and args.model != "transformer" and args.model != "molclr":
        args_to_remove |= {"init_lr", "max_lr", "final_lr", "precision"}
    if args.model != "nn" and args.model != "mpn" and args.model != "transformer" and args.model != "molclr":
